...
SERVICE_NAME=$1
PROJECT_NAME=mlw-tongji
RELEASE_DIR=/data/compileAnddeploy/testEnv/release
SRC_DIR=/data/compileAnddeploy/testEnv/source
APP_WORK_DIR=/data/software/mlwsvr
TO_IP="10.14.16.54"

if [ -z $SERVICE_NAME ];then
  echo "Please define an async module name like tongji_service."
  exit 1
fi
...
for ip in $TO_IP
  do 
    ...
    echo "scp $RELEASE_FILES $SRC_DIR/$PROJECT_NAME/target/lib/* root@$ip:/data/software/mlwsvr/$SERVICE_NAME/lib/"
    scp $RELEASE_FILES $SRC_DIR/$PROJECT_NAME/target/lib/* root@$ip:/data/software/mlwsvr/$SERVICE_NAME/lib/
...
if [ "y" == "$rbk" -a ! -z "$RBK_IP" ];then
  ...
  echo "ssh root@$ip $APP_WORK_DIR/rollback.sh $SERVICE_NAME"
  ssh root@$ip "$APP_WORK_DIR/rollback.sh $SERVICE_NAME"
fi
