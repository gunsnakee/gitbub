...
<form id="pagerForm" method="post" action="/oms/service/change">
    <input type="hidden" name="pageNum" value="${pc.pageInfo.page}"/>
    <input type="hidden" name="numPerPage" value="${pc.pageInfo.pagesize}"/>
</form>
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="/oms/service/change" method="post"
          id="ret_change_form" >
        <div class="searchBar">
			<table class="searchContent pageFormContent">
				<tr>
					...
					<c:forEach var="e" items="${changeStatus}">
				...
				<tr>
					...
					<td>
						...
						<button id="ret_change_clean">重置</button>
					</td>
                </tr>
			</table>
        </div>
        ...
        <c:if test="${!empty pc.entityList}">
            ...
            <c:forEach var="e" varStatus="i" items="${pc.entityList}">
                ...
                <tr target="orderItemId"  rel="${b.orderItemId}">
					...
					<c:forEach var="o" varStatus="i" items="${changeStatus}">
					...
					<td>
						...
						<p style="padding: 3px;">
                            <a title="你确认要收货吗？" target="ajaxTodo" href="/oms/service/changeReceive?retordItemId=${b.orderItemId}" ><span style="color: #0000ff">确认收货</span></a>
						</p>
						<p style="padding: 3px;">
						...
						<c:if test="${b.orderItemStatus==40}">
                            <p style="padding: 3px;">
                            <a title="输入物流信息" target="dialog" href="/oms/service/ship?ids=${b.orderItemId}"><span style="color: #0000ff">确认发货</span></a>
                        </p>
						<%--<p style="padding: 3px;">--%>
                            <%--<a title="你确认要发货吗？" target="ajaxTodo" href="/oms/service/send?retordItemId=${b.orderItemId}" ><span style="color: #0000ff">确认发货</span></a>--%>
						<%--</p>--%>
						</c:if>
						<c:if test="${b.orderItemStatus==50}">
						<p style="padding: 3px;">
                            <a title="客户已经收到货了吗？" target="ajaxTodo" href="/oms/service/clientReceive?retordItemId=${b.orderItemId}" ><span style="color: #0000ff">买家确认收货</span></a>
						</p>
						<p style="padding: 3px;">
                            <a title="你确认要发起协商吗？" target="dialog" href="/oms/service/consultation?retordItemId=${b.orderItemId}" ><span style="color: #0000ff">买家发起协商</span></a>
						</p>
						</c:if>
						...
						<p style="padding: 3px;">
                            <a title="查看处理" target="navTab" href="/oms/service/detailChg?retordItemId=${b.orderItemId}"><span style="color: #0000ff">查看处理</span></a>
						</p>
...
$("#ret_change_clean").click(function(check){
	$("#ret_change_form input").each(function(i){
		var val = $(this).val("");
			
	});
	
	$("#ret_change_form select").each(function(i){
		$(this).val("0")
	});
