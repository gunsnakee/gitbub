...
<form id="pagerForm" method="post" action="/tv/addPro">
...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return ThemeProNavTabSearch(this);" action="/tv/addPro" method="post"
   ...
   <div class="panelBar">
        <ul class="toolBar">
            <c:if test='${sessionScope.bkstage_user.menus["31adbbaf1dc5d89f7ee25787814e66bf"]!=null}'>
                <li><a title="确实要把商品加入专题页?" target="selectedTodo" rel="ids" href="/tv/addPro?id=${id}&handle=1" postType="string" class="add"><span>添加商品到专题页</span></a></li>
            </c:if>
        </ul>
   </div>
        ...
        <tbody>
            ...
            <c:if test="${!empty pc.entityList}">
                <c:forEach var="e" varStatus="i" items="${pc.entityList}">
                    <tr>
                        <td>
                            <c:if test="${e.isDel == 1}">已选择</c:if>
                            <c:if test="${e.isDel != 1}">
                            <input name="ids" value="${e.proId}" type="checkbox"></c:if>
                        </td>
                        <td>${e.proId}</td>
                        <td>${e.proName} ${e.skuName}</td>
                        <td>${e.mlwPrice}</td>
                        <td>
...
$(document).ready(function () {
    ...
    $("#theme_list_reset").click(function (check) {
        ...
        $("#themeAdd_list_queryForm select").each(function(i){
            var $this = $(this);
