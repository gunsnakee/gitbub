        ...
        onSelect:function (g) {
            ...
            for (var f in this.queueData.files) {
                ...
                if (e.uploaded != true && e.name == g.name) {
                    var d = confirm('文件"' + g.name + '"已经存在上传队列中.是否需要把它重新替换？');
                    if (!d) {
                ...
                switch (g) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (e.queueSizeLimit > f) {
                            this.queueData.errorMsg += "\n选择的文件数量超过了上传限制 (上传数量限制为：" + f + ").";
                        } else {
                            this.queueData.errorMsg += "\n选择的文件数量超过了队列的大小限制 (队列大小限制为：" + e.queueSizeLimit + ").";
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 大小超过限制 (请上传小于' + e.fileSizeLimit + "的文件).";
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '"为空.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\n文件 "' + d.name + '" 类型不符合规定 (只能上传' + e.fileTypeDesc + "这些类型的文件).";
                        break;
                }
            }
        ...
        onUploadStart:function (d) {
            ...
            if (e.checkExisting) {
                c.ajax({
                    ...
                    success:function (h) {
                        if (h == 1) {
                            var g = confirm('此文件"' + d.name + '" 已经存在服务器上.\n是否替换？');
                            if (!g) {
