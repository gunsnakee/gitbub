$(function(){
	$("#redis_keys_header a").each(function(){
	...
	function searchListener(e){
		...
		var uuid = $.trim($("#redis_uuid_select").val()) ;
		...
		var url = '/redis/keys/getByPattern.htm';
...
function getValue(key){
	...
	var url = '/redis/keys/value.htm?key=' + key +'&uuid=' + uuid ;
...
function updateString(){
	...
	$.post('/redis/keys/updateString.htm' , {key:key , value:val} , function(data){
		console.log(data) ;
		if(data.status == 0){
			message('修改成功') ;
		} else {
...
function deleteString(){
	$.post('/redis/keys/deleteString.htm' , {key:key} , function(data){
		console.log(data) ;
		if(data.status == 0){
			message('删除成功') ; 
		} else {
