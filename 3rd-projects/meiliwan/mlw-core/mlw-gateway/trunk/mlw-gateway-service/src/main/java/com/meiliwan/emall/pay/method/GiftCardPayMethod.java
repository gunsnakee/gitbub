...
public class GiftCardPayMethod extends InnerPayMethod {
	private static final MLWLogger LOG = MLWLoggerFactory.getLogger(GiftCardPayMethod.class);
	...
	public PayStatus payMoney(PaymentDTO payment, PayParam payParam, String password, String clientIP) {
		...
		double result = GiftCardClient.subMoney(payment.getOrderId(), payment.getUid(), payParam.getAmount(), password, payParam.getPayId());
		...
		LOG.info("giftCard pay","payResult:"+payStatus+ ", payParam" + payParam.allToString(), "");
	...
	public PayStatus robackMoney(PaymentDTO payment, PayParam payParam) {
		...
		walletDto.setuId(payment.getUid());
		walletDto.setMoney(payParam.getAmount());
		double result = GiftCardClient.updateRefundFromGateway(walletDto);
