 ...
 */
@Component
public class OrdIStatusFinished extends OrderStatusService{
	...
	public List<OrdiStatus> getOrdiStatus(OrderStatusDTO ordStatusDIO) throws ServiceException{
		return ordiStatus2List(ordiStatusDao.selectByOrdIStatus(ordStatusDIO.getOrderItemId(), 
				IS.getType(), ORDI_RECEIPTED.getCode()));
	}
	...
	protected void updateStatus(OrderStatusDTO ordStatusDIO,
		...
		//修改状态做已生效
		ordiStatusDao.updateOrdIStatus(ordStatusDIO.getOrderItemId(), IS.getType(), ORDI_FINISHED.getCode());
		
		//更新订单行表的状态
		updateOrdIAndOrdStatus(ordStatusDIO, ORDI_FINISHED);
	}
