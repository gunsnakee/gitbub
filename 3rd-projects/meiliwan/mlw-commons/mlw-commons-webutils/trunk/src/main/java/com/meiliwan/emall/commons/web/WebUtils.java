...
public class WebUtils {
    ...
    public static void writeStringToResponse(String content, HttpServletResponse response){
    	try {
    		...
    		logger.error(e, IPUtil.getLocalIp() + " write response ioe", "");
    	}
    }
     ...
     */
    public static void writeStringToResponse(String content, HttpServletRequest request, HttpServletResponse response) throws ServletRequestBindingException{
    	try {
    		...
    		logger.error(e, IPUtil.getLocalIp() + " write response ioe", getIpAddr(request));
    	}
    }
...
//            response.setContentType("text/xml");
    ...
    public static String getClientIp(HttpServletRequest request){
    	...
    	return StringUtils.isNotBlank(clientIp) ? clientIp.split(",")[0] : "127.0.0.1";
    }
     ...
     * @param response
     */
    public static String getEnCode(HttpServletRequest request, HttpServletResponse response, String enCode){
    	String fmToken = "" ;
        ...
        try {
            ...
            int randNum = 0;
            if(StringUtils.isBlank(cacheRandCode)){
                //先随机一个4-10的随机数，作为随机因子，再以此因子位randCode的随机长度
                randNum = RandomUtil.randomNumRange(4, 10);
                randCode = RandomUtil.randomStrCode(randNum);
                //将随机码写入redis
            ...
            logger.debug("enCode="+enCode+", sessionId="+sessionId+", randCode="+randCode+", randNum="+randNum);
            enCode = ValidateFormUtil.getEncryptCode(enCode, randCode);
            fmToken = ValidateFormUtil.getNoRepeatSubmitCode(request, response, "");
            
        }catch (Exception e) {
            logger.error(e, fmToken, WebUtils.getIpAddr(request));
        }
