...
//    private final MLWLogger logger = MLWLoggerFactory.getLogger(this.getClass());
    ...
    private boolean checkLogin(HttpServletRequest request, HttpServletResponse response){
        ...
        if (uid == null) {
            request.setAttribute(WebConstant.IS_LOGINED, WebConstant.FALSE);
            UserCookieUtil.updateNickName(request, response, "");
            WebUtils.deleteCookie(request, response, "/", "_ml");
            return false;
        } else {
            request.setAttribute(WebConstant.IS_LOGINED, WebConstant.TRUE);
            request.setAttribute(WebConstant.UID, uid);
            return true;
        } 
    }
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if(checkLogin(request,response)){
            ...
            response.sendRedirect(response.encodeRedirectURL(WebConstant.LOGIN_URL_VALUE + "?targetUrl=" +targetUrl));
            return false;
        }
    ...
    public void postHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        if("post".equals(handle)){
            if (!checkLogin(request, response)) {
                response.sendRedirect(WebConstant.LOGIN_URL_VALUE + "?targetUrl=http://www.meiliwan.com?g="+System.currentTimeMillis());
                return;
            }
        }
