...
public class NumberUtil {
	...
	private final static DecimalFormat DOUBLE_FORMAT_DEFAULT = new DecimalFormat(
			"0.00");
	...
	/**
	 * 
	 * 根据key和分摊比例，将p值分摊到每一个key上，最后一个key的值为 sum - sumAmortizeV(lastKey-1)
	 * 
	 * @param ratioOfIdMap 
	 * @param p
	 * @return
	...
	public static Map<Integer, Double> amortizeByPriceRatio(Map<Integer, Double> ratioOfIdMap, double p) {
		if (ratioOfIdMap == null || ratioOfIdMap.isEmpty() ) {
			return Collections.EMPTY_MAP;
		}
		
		if (p <= 0) {
			throw new IllegalArgumentException(String.format("amortize value s% is not valid", p));
		}
		
		Map<Integer, Double> rtKeyPriceMap = new HashMap<Integer, Double>();
		BigDecimal sum = new BigDecimal(0);
		for (Integer  keyId : ratioOfIdMap.keySet()) {
			sum = sum.add(BigDecimal.valueOf(ratioOfIdMap.get(keyId)));
		}
		
		double alreadySum = 0.0;
		int i = 0;
		int limit = ratioOfIdMap.keySet().size() - 1;
		...
		for (Integer  keyId : ratioOfIdMap.keySet()) {
			...
			double oRatio = ratioOfIdMap.get(keyId);
			double v = p * (NumberUtil.divide(oRatio, sum.doubleValue(), 16));
			rtKeyPriceMap.put(keyId, v);
			alreadySum += v;
			i++;
		}
		
		double lastV = p - alreadySum;
		rtKeyPriceMap.put(lastKey, lastV);
		
		return rtKeyPriceMap;
	}
