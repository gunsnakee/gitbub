...
public class UserCookieUtil {
    ...
    public static boolean writeLoginCookie(HttpServletRequest request, HttpServletResponse response, String loginType, String loginName, String nickName,Integer uid) {
        ...
        WebUtils.setCookieValue(response,"_ml", UserLoginUtil.uidToMl(uid),"/",UserCommon.DOMAIN,-1);
        ...
        try {
            mu = URLEncoder.encode(mu, UserCommon.CHARSET_ENCODE);
        ...
        try {
            WebUtils.setCookieValue(response,MU, mu,"/",UserCommon.DOMAIN,UserCommon.MU_TIIME);
        }catch (Exception e){
            LOGGER.error(e, e.getMessage() + " mu:" + mu, WebUtils.getIpAddr(request));
            throw new BaseRuntimeException(ErrorPageCode.EXCEPTION_SYSTEM_DEFAULT);
        }
    ...
    public static void updateNickName(HttpServletRequest request, HttpServletResponse response, String nickName) {
        ...
        if(StringUtils.isNotBlank(mu)){
            String newVal = "";
            try {
                mu = URLDecoder.decode(mu,UserCommon.CHARSET_ENCODE);
            } catch (UnsupportedEncodingException e) {
                LOGGER.error(e, e.getMessage() + " mu:" + mu, WebUtils.getIpAddr(request));
                throw new BaseRuntimeException(ErrorPageCode.EXCEPTION_SYSTEM_DEFAULT);
            }
            ...
            try {
                newVal = URLEncoder.encode(newVal, UserCommon.CHARSET_ENCODE);
            } catch (UnsupportedEncodingException e) {
            ...
            try {
                if(StringUtils.isNotBlank(newVal)){
                    ...
                    WebUtils.setCookieValue(response, MU, newVal, "/", UserCommon.DOMAIN, UserCommon.MU_TIIME);
                }
            }catch (Exception e){
                LOGGER.error(e, e.getMessage() + " newVal:" + newVal, WebUtils.getIpAddr(request));
                throw new BaseRuntimeException(ErrorPageCode.EXCEPTION_SYSTEM_DEFAULT);
