 ...
 * <ul>
 * <ol>
 * 沿用了{@link java.util.concurrent.ConcurrentHashMap}的分离锁策略，在将内部其内部将桶划分为若干个分区（{@link com.meiliwan.emall.commons.util.concurrent.ConcurrentLRUHashMap.Segment}）,每个分区都是一个基于lru策略的
 ...
 * </ul>
 * 该实现不是无阻塞的。
 * 该实现返回的所有迭代器都不会抛出{@link java.util.ConcurrentModificationException}异常，但是通过该迭代器获取到的结果是“弱一致性”的。
 * 
 * @param <K>
 * @param <V>
 */
public class ConcurrentLRUHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>,
    ...
    /**
     * segment实际上是{@link java.util.Hashtable}的一种实现，该类继承了{@link java.util.concurrent.locks.ReentrantLock}
