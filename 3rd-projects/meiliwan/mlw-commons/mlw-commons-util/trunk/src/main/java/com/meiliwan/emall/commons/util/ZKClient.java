...
import com.meiliwan.emall.commons.util.concurrent.JMXConfigurableThreadPoolExecutor;
...
public class ZKClient
{
	...
	private static final MLWLogger log = MLWLoggerFactory.getLogger(ZKClient.class);
					...
					if(this.triggerInNewThread())
					{
						ZKClient.get(). watchExecutorService.submit(new Runnable()
						{
							...
							public void run()
							{
								...
								{
									log.error(e, "Error while submit node added:{}",  fs );
								}
							}
						});
					}
	...
	private void init()
		...
		zooKeeper = new ZooKeeperRetry(StringUtils.join(zkHostList, ","), new Integer(BaseConfig.getValue("zk.session.timeout", "30000")), defaultWatch);
			}
	...
	public void registerEphemeralNode(String path, String nodename)
		...
		if(nodename==null)
		{
			throw new NullPointerException("hostname is null,path:"+path);
		}
	...
	private void __watchChildren(final WatchBag bag)
	{
		...
		try
		{	Stat stat=new Stat();
		List<String> result = zooKeeper.getChildren(bag.path,w ,stat);
	...
	public void addWatch(WatchBag bag)
	{
		...
		if (watch instanceof MasterWatcher)
		{
			this.watchMaster(bag.path, bag.payload, (MasterWatcher) watch);
		}
		else if (watch instanceof LongValueWatcher)
		{
			this.watchLongValueNode(bag.path,  (LongValueWatcher) watch);
		}
		else if (watch instanceof ChildrenWatcher)
		{
			watchChildren(bag.path,  (ChildrenWatcher) watch);
		}
		else if (watch instanceof StringValueWatcher)
		{
	...
	private void deleteMaster(final String path, final String serverAddress )
	{
		if (serverAddress != null)
		{
			...
			{
				log.error(e, "Error while deleteMaster (line 1014): path{},address:{}", path + serverAddress);
		...
		final  Watcher w = new Watcher()
		{
			public void process(org.apache.zookeeper.WatchedEvent we)
			{
				...
				else if (we.getType() == Event.EventType.NodeDataChanged)
				{
					...
					log.error(new Exception("Node data should not change"), "Node data should not change:{}", "");
				}
			}
		};
