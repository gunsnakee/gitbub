...
public class KafkaSink extends AbstractSink implements Configurable{
	...
	private AtomicInteger serialNum = new AtomicInteger(1);
	private final static String LOCAL_IP_SUFFIX = KafkaSinkUtil.getLocalIpSuffix();
	...
	public Status process() throws EventDeliveryException {
		...
		try {
			...
			if(serialNum.get() > 10000){
				serialNum.set(1);
			}
			
			String accessLog = new String(e.getBody());
			accessLog = accessLog.replaceFirst("tm\\(.*\\)ip", "tm(" + System.currentTimeMillis() + "-" + LOCAL_IP_SUFFIX + "-" + serialNum.getAndIncrement() + ")ip");
			producer.send(new KeyedMessage<String, String>(topic, accessLog));
			tx.commit();
			return Status.READY;
		} catch(Exception e) {
