	...
	private static class ABTest{
		...
		public ABTest(JsonObject ab){
			...
			for(Entry<String, JsonElement> kv: ab.entrySet()){
				...
				ratioSum += values.get(Constants.ABTEST_RATIO).getAsDouble();
				JsonObject paraMap = values.getAsJsonObject(Constants.ABTEST_PARAM);
                String abbr = values.get(Constants.ABTEST_ABBR).getAsString();
...
//			logger.warn("PATH NOT FOUND ON ZK");
	...
	public boolean load(String json){
		...
		JsonObject ab = jo.get(Constants.ABTEST_STRING).getAsJsonObject();
	...
	public String toString(){
		...
		result.addProperty(Constants.ABTEST_STRING, abTest.toString());
