...
public class SourceOrderSaleCounter implements StatWorker {

	private final static MLWLogger LOG = MLWLoggerFactory
			.getLogger(SourceOrderSaleCounter.class);

	SourceChecker checker;

	public SourceOrderSaleCounter() {
	...
	public Map<String, Integer> stat(List<PageView> pvs) {
		...
		for (int i = source.indexOfList; i < pvs.size(); i++) {
			...
			if (visit.contains("/gateway/pay")
				...
				LOG.info("visit url:", visit + ",source:" + source.sourceSite,
						null);
		...
		int saleOrder = 0;
		if (!orders.isEmpty()) {
			String sql = "select  sum(order_sale_amount)  from oms_ord where order_id in ('"
			...
			try {
				...
				while (rs.next()) {
					saleOrder = (int) Math.round(rs.getDouble(1) * 100);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		// LOG.debug("source:" + source.getSourceSite() + ",orders:" + orders +
		// ",saleOrder:" + saleOrder);
		result.put(source.sourceSite, saleOrder);
		return result;
	}
